...
bool linear(int [], int, int );//Does a linear search

bool linear(float [], float, float);//Does a linear search

void vectors (vector<float> &); //This is the vector that will assign values to the
                            //large game
void exit();
...
string title[wel]={"Welcome", "Picture"};

string title2[wel]={"to", "Poker"};
//end of option 1-------------------------------------------------------------
...
if (again!='y')
    ...
    cout << "\nThank you " << name << " for your participation";
     ...
     bool leave;
    bool leave=linear(deck, cardta, 10);
    if (leave==false){
    exit();
    }
...
//This vector function will pass by a value of 5 cards 
void vectors (vector<float> & three){
...
//This function checks to see if a specific number is there
bool linear (int array[], int select, int need)
{
...
bool linear (float array[], float select, float need)
{
    for (int x=0;x<select;x++){
        if (need==array[x]){
            return true;
